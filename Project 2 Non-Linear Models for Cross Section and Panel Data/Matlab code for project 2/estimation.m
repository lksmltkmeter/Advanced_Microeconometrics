classdef estimation
    ...
    methods (Static)
        ...
        function [thetahat,se_thetahat] = estimate_m(q,theta0,N,opt,strCovType)
            %estimate_m(q,theta0,N,opt_search,strCovType)
            ...
            %[thetahat,fval,exitflag,output] = fminunc(Q,theta0,opt_search)


            % 3. asymptotic variance matrix
            % get gradients in N*K vector form
            s = estimation.centered_grad(q,thetahat); % s is N*K
            B = s'*s/N; % s'*s is K*K
            %for fminunc
            ...
            % 5. print results
            estimation.print_results(thetahat,se_thetahat);
