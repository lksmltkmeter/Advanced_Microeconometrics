classdef estimation_clog
    ...
    methods (Static)
        ...
        function [thetahat,se_thetahat,out] = estimate_m(q,theta0,N,options,strCovType) 
            ...
            s = estimation_clog.centered_grad(q,thetahat); % s is N*K 
            
            B = s'*s/N; % s'*s is K*K
            A = hess/N; % alternatively, "estimation.hessian(q,thetahat)/N"
            %A = estimation.hessian(q,thetahat)/N; 
            switch strCovType
                case 'Sandwich'
                    cov = 1/N * (A\B/A);
                case 'Outer Product'
            ...
            % 5. print results 
            estimation_clog.print_results(thetahat,se_thetahat); 
        ...
        function [thetahat,se] = estimate_model(model,y,x,options,strCovType)
            ...
            [thetahat,se] = estimation_clog.estimate_m(q,theta0,N,options,strCovType); 
        end; 
        
        function [thetahat,se_thetahat,out] = estimate_bhhh(model,y,x,options,theta0)
            q = @(theta) model.criterion(y,x,theta); 
            Q = @(theta) estimation_clog.crit_for_bhhh(q,theta); 
        ...
        function [ Q,g,h ] = crit_for_bhhh(q,theta)
            Q = sum(q(theta)); 
            s = estimation_clog.centered_grad(q,theta); 
            h = s'*s; 
            g = sum(s,1); 
        end; 
        ...
        function [ grad ] = centered_grad( fun , x0 , h )
            ...
            if nargin<3; 
                h = estimation_clog.stepsize_grad; 
            end; 
        ...
        function hess = hessian( fhandle , x0 )
            ...
            h_rel = estimation_clog.stepsize_hess; 
